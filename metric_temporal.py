...
def evaluate_images(image_id, metrics=None, vortex_data=None, api_key=None):
    """
    Evaluate temporal reasoning images based on specified metrics
    """
    ...
    for metric in metrics:
        if metric == "reasoning_process":
            prompt_text = prompt_reasoning_process_temporal.format(
                prompt=prompt,
                dimension=dimension,
            ...
            prompt_text = prompt_reasoning_visual_temporal.format(
                prompt=prompt,
                dimension=dimension,
                keywords=keywords_str,
                target_description=target_description
...
def run_vortex_evaluation(
    ...
    """
    Run VortexBench evaluation for all tasks
    """
    metrics = metrics or METRICS
    ...
    # Load VortexBench data
    if not os.path.isfile(VORTEX_DATA_FILE):
        logging.error(f"VortexBench data file not found: {VORTEX_DATA_FILE}")
        return
    
    try:
        with open(VORTEX_DATA_FILE, 'r', encoding='utf-8') as f:
            vortex_data = json.load(f)
    except Exception as e:
        logging.error(f"Error loading VortexBench data {VORTEX_DATA_FILE}: {e}")
        return
    
    # Filter temporal tasks only
    temporal_tasks = [task for task in vortex_data["tasks"] if task.get("reasoning_type") == "temporal"]
    expected_keys_map = {task["id"]: task for task in temporal_tasks}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="VortexBench Temporal Reasoning Evaluation")
