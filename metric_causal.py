    ...
    for metric in metrics:
        if metric == "reasoning_process":
            prompt_text = prompt_reasoning_process_causal.format(
                ...
                think_output=think_output if 'think_output' in locals() else "No think output available"
            )
...
def run_causal_evaluation(
    ...
    """
    Run causal reasoning evaluation for a specific model and dimension
    """
    metrics = metrics or METRICS
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="VortexBench Causal Reasoning Evaluation")
